...
You need to allow the users to configure the module. And for that your module should have the configure link at the back office under Module and Services tab, in the module list. 

The "Configure" link appears when you add `getContent()` method to your main class. This is the standard QloApps method, its purpose is to inform the back office that the module consists a configuration page and it has to display the configuration link.  

But `getContent()` public method in the module object only makes the link appear, not create a configuration page. You will need to create a configuration page, and we will explain how. 
...
When the configuration page is loaded the `getContent()` method is called first. And that is why it is used to first update any value that is submitted by any form that exists on configuration page. 

Here is a line by line explanation:

1. `Tools::isSubmit()` is a method to check the validity of indicated form.
...
1. `Tools:getValue()` retrieve the content of the POST or GET array in order to get the value of the specified variable.
...
The Validate object has many data validation methods, one of which is `isGenericName()`, this method helps you keep only strings that are valid QloApps name i.e. a string that does not contain special characters, for short.
...
As we saw earlier, the Configuration object has just the `updateValue()` method which will will store the new value for module_name in the configuration data table.
...
1. At the end, we use the custom `displayForm(`) method in order to add content to $output and return that content to the page.
...
At the end when everything is setup, we can call the `generateForm()` method, which will take care of putting it all together and, as its name says, generate the form that the user will use to configure the module's settings.
